...
export class CommandExecutor {
    ...
    handleMouseMoveRelative(command) {
        let clamped = false;
        ...
        return this.createResult(clamped ? 'SUCCESS_WITH_CLAMP' : 'SUCCESS', clamped ? `${baseMessage} Clamped to workspace edges.` : baseMessage);
    }
    handleMouseMoveAbsolute(command) {
        let clamped = false;
        this.state.update((draft) => {
        ...
        return this.createResult(clamped ? 'SUCCESS_WITH_CLAMP' : 'SUCCESS', clamped ? `${baseMessage} Clamped within workspace.` : baseMessage);
    }
    handleMouseClick(command) {
        var _a;
        ...
        const target = (_a = hitTest === null || hitTest === void 0 ? void 0 : hitTest.element) !== null && _a !== void 0 ? _a : null;
        if (!target) {
    ...
    getWorkspaceElement(doc) {
        return doc.querySelector('.desktop-pane');
    }
    getCursorViewportPoint(doc) {
    ...
    getHitTarget(doc) {
        ...
        return { element: target !== null && target !== void 0 ? target : null, point };
    }
    dispatchClickSequence(target, type, point) {
    ...
    dispatchPointerEvent(target, type, init) {
        if (typeof PointerEvent === 'undefined') {
            return;
        }
        target.dispatchEvent(new PointerEvent(type, { ...init, bubbles: true, cancelable: true }));
    }
    dispatchMouseEvent(target, type, init) {
        target.dispatchEvent(new MouseEvent(type, { ...init, bubbles: true, cancelable: true }));
    }
    createMouseEventInit(point, button) {
        var _a, _b;
        return {
            button,
            clientX: (_a = point === null || point === void 0 ? void 0 : point.x) !== null && _a !== void 0 ? _a : 0,
            clientY: (_b = point === null || point === void 0 ? void 0 : point.y) !== null && _b !== void 0 ? _b : 0,
        };
    }
    ...
    isTextAcceptingElement(element) {
        if (!element) {
            return false;
        }
    ...
    injectText(element, text) {
        var _a, _b;
        if (element instanceof HTMLTextAreaElement || element instanceof HTMLInputElement) {
            const start = (_a = element.selectionStart) !== null && _a !== void 0 ? _a : element.value.length;
            const end = (_b = element.selectionEnd) !== null && _b !== void 0 ? _b : element.value.length;
        ...
        if (element instanceof HTMLElement && element.isContentEditable) {
            element.focus();
            const doc = this.getDocument();
            doc === null || doc === void 0 ? void 0 : doc.execCommand('insertText', false, text);
        }
    }
    dispatchKeyCombo(target, command) {
    ...
    createModifierKeyboardInit(modifier) {
    ...
    createKeyboardInit(key, modifiers) {
        return {
            key,
    ...
    describeElement(element, doc) {
        ...
        if (labelledBy) {
            ...
            if (labelEl === null || labelEl === void 0 ? void 0 : labelEl.textContent) {
